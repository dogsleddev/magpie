...
_Update 2026-07-12: five commits on top (`eb75165` .. `44399ef`), committed but NOT pushed (a push rebuilds the preview, gated on Chris). Done: the no-gate audit cleanups (Library revalidation, honest Nest error handling, the `+ add` reuse typeahead, the §3.7 spec fix), the "see as nest" reverse jump (verified in-browser), and `scripts/merge-entities.mjs`. **Two investigations changed the plan (see picks 3 and 5): the 36-topic backfill hole is already closed (1 left), and there are ~no clean dupes to merge (the audit's dupes are actually nesting pairs). Do not run a merge or backfill `--write`.** The real noise is auto-nesting zero-glint hubs (§6.5). Slice 4 plus the Chris-gated preconditions (security rotation, DB snapshot) remain the open front. Completed items carry `[DONE 2026-07-12]` markers below._
