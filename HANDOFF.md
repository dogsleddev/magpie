...
_Updated 2026-07-11 (session end). **The entity-spine redesign is BUILT through Slice 3** on branch `feat/magpie-2` (all pushed, preview-only). The daily loop is real end to end: catch a glint in your words, correct or split its entities, watch it connect honestly through a shared hub, and browse it all in the Library and the Nest. **REDESIGN COMPLETE 2026-07-13: Slice 4 (the Maggie tab-merge) shipped and is LIVE at magpie.wiki.** The topic page now lands on Brief, Brief/Challenge/Questions carry Favorite + Dismiss, and the persona tab moved last and absorbed Thoughts (a Talk/Notes toggle over one shared notes store; "keep" a chat line to pin it). Verified live on prod. All four slices are now on `master`, which IS 2.0. Read top to bottom, then the "What to do next" section drives the session._

_Update 2026-07-12: twelve commits (`eb75165` .. `62d4f9c`), all pushed AND merged to `master` (clean fast-forward), so 2.0 is now LIVE at magpie.wiki. The final commit rewrote the landing for the glint-first story (it was still selling 1.0 Subject/Topic/Facet + "five modes"); the merge was gated on that. Done: the no-gate audit cleanups (Library revalidation, honest Nest error handling, the `+ add` reuse typeahead, the §3.7 spec fix), the "see as nest" reverse jump (verified in-browser), and `scripts/merge-entities.mjs`. **Two investigations changed the plan (see picks 3 and 5): the 36-topic backfill hole is already closed (1 left), and there are ~no clean dupes to merge (the audit's dupes are actually nesting pairs). Do not run a merge or backfill `--write`.** The real noise was auto-nesting zero-glint hubs (§6.5), now fixed at the root by tightening the extractor's `broader` rule (`509fea1`+`5995995`, empirically calibrated); nesting stays automatic, no capture-flow cost. Slice 4 plus the Chris-gated preconditions (security rotation, DB snapshot) remain the open front. Completed items carry `[DONE 2026-07-12]` markers below._

_Update 2026-07-13: **Slice 4 shipped to prod** (`master` at `ebbf060`, verified live). The topic tab-merge: tabs reorder to Brief/Challenge/Questions/Maggie and land on Brief; Favorite pins a response to the topic's notes and Dismiss rerolls; the Maggie tab holds a Talk/Notes toggle over one shared `useThoughts` store (lifted to `ModeTabs`), with a "keep" action on chat lines. New: `use-thoughts.ts`, `notes-list.tsx`, `maggie-tab.tsx`; retired `persona-mode.tsx`. No migration (reuses `thoughts`). An adversarial review found + I fixed duplicate-write bugs (dedup in the store; optimistic keep guard). **Chris's product call: the old riff timer and Organize-with-AI were dropped on purpose (1.0 artifacts, not 2.0 gameplay; synthesis lives in the chat now), not to be resurrected as buttons.** Two known-and-left items: editing an in-flight (temp) note within its save window can revert (pre-existing); and the `default_mode` DB CHECK lacks `'maggie'` (harmless, nothing writes it, fix that migration when a Settings screen is rebuilt). The redesign (Slices 1-4) is DONE and live; the open front is now the Chris-gated security rotation, which unblocks per-user auth and the daily email/text trigger (the retention loop)._
...
- **[RESOLVED 2026-07-12, `509fea1` + `5995995`] Auto-nesting §6.5: fixed at the root, kept automatic.** The old code auto-wrote `broader` at capture, minting a broader hub with no undo. Evidence (`merge-entities.mjs --ghosts`): 22 zero-glint hubs, ~half valuable rollups (`sports` gathers 11, `cities`/`film`/`literature` 5 each) and ~half single-child noise (`mollusks`->octopuses, `toys`->lego). A one-tap "propose not automatic" prototype (`4a1dabd`) was built then **reverted after pushback**: it taxed the 30-second catch, its gate was a leaky proxy, and it did not fix the cause. The cause is the extractor proposing an over-granular `broader` for a single glint, so the fix is the prompt: `categorizeTopicPrompt` now returns a `broader` only for a big common category (basketball->sports, octopuses->animals) and null/upgrade for narrow taxonomic or junk-drawer parents (mollusks->animals, lego->null). **Verified empirically** on the real ghost cases (Haiku calls, no writes). Nesting stays fully automatic (pure-auto) with no capture-flow cost and no new single-child ghosts. The 11 existing single-child ghosts are untouched; most are legit-but-early categories (planets, philosophers, diseases) that self-heal, so a prune stays optional. `--ghosts` is kept as a standing detector.
