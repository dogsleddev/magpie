...
The five topic tabs ({persona} / Brief / Challenge / Questions / Thoughts) collapse into one Maggie thread where Brief/Challenge/Questions become **offered moves inside the conversation**, not destinations. On open Maggie leads with a brief-as-opener (seeded from `brief_seed`), lowercase, then three suggestion chips: `[ push me ]` (Challenge, Sonnet), `[ ask me things ]` (Questions, Haiku), `[ i'll just talk ]` (free capture).

**Code reality (verified 2026-07-12, drives the Slice 4 plan).** In the shipped code (`components/topic/mode-tabs.tsx`) the persona-named tab renders `ConvoMode` (the Maggie chat) and the fifth tab is **Thoughts**, which renders `PersonaMode` (the bullet capture). So the Maggie thread this slice merges into **already exists as tab 1**, and the bullet surface to preserve is the Thoughts tab, not a separate "Convo". This makes the merge additive: fold Brief/Challenge/Questions into the tab-1 thread as inline chips, demote Thoughts to a linked notes view, then retire the middle tabs. No new chat storage is needed (`conversations.messages` already backs tab 1).

**Do not blanket-write chat into `thoughts` (resolves critique #5).** The Thoughts tab (`PersonaMode`) captures deliberate curated bullets; writing every conversational turn into `thoughts` would pollute the very "come back to your own words" notes surface the merge preserves. Rule: **only turns entered under `i'll just talk` are saved as thoughts.** Ordinary replies to Maggie's Challenge/Questions are conversation, not bullets. The notes list stays curated.
