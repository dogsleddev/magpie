...
- **Open right now:** publish the **LinkedIn post** (draft + reviewer prompt in `docs/LINKEDIN_LAUNCH.md`; the og image is DONE, it only needs the people to tag). Verify the iOS keyboard-dictation hint and the mobile Nest default on Chris's iPhone. Curate the near-duplicate community topics (confirmed pair: the two Yellowstone-wolves topics; propose a merge list before deleting anything). Privacy stays **off the site** (Krava is Level-1 inference-TEE only, storage plaintext, prod routing unverified). The carryover QC backlog + the `service_role` -> `sb_secret_` migration (deprecated **end of 2026**) are in `PROGRESS.md`.
