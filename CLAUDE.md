...
- **The Nest mind map is built and shipped.** It replaced the Journal tab: a force-directed constellation of the whole wiki with three dimensions (Subject to Topic to Sub-topic containment, the cross-subject Facet web, and emergent Resonance). Full details in `docs/NEST.md`. Live at the `/nest` tab and embedded on the landing. It has a **Desktop** toggle (full-screen overlay + docked control panel) and a `subjectsOutside` option. **On desktop-size viewports (>=1024px), Desktop mode opens automatically** (SSR-safe mount effect in `components/nest/nest-view.tsx`).
- **Entity groups (session 11, live):** topics that belong to the same named entity (book series, team, show, franchise) now nest under a shared group parent in the Nest. **Red Rising** and **Corvids** were backfilled as groups on the live community DB via `scripts/group-backfill.mjs`. Every new topic added via Magpie runs the **umbrella check** (`applyUmbrella` in `lib/actions/topics.ts`): the categorizer identifies entity siblings in the existing topic list and auto-nests them under one group parent, creating the parent if needed. Conservative: never creates a group of one; failures log but never block the add.
- **Homepage (session 11, live):** nav CTA contrast fixed (specificity bug), hero copy tightened, "See the community nest" is the first highlighted button, "See the community topics" replaces the old waitlist button, five-modes section rewritten to match the real app tabs, Maggie card trimmed.
- **Also shipped previously (all live):** **Rediscover** (bottom-bar tab spins to a random topic), **Maggie's per-topic AI opener** (`/api/ai/convo-opener`, in-memory cached), **Sports** subject, facet chips that link to `/facets/[id]`, quiet **delete-topic**, mic-to-text, Organize, the Krava privacy wrap, the Linq Tier 0 webhook, landing Nest showcase + inline waitlist, og/twitter metadata, in-app header waitlist CTA, footer contact pointing to Chris's LinkedIn.
...
- **iOS mic, new approach (live, unverified on-device):** WebKit's Web Speech is a dead stub on iOS, so we **hide the in-app mic on iOS** and show a "tap the mic on your keyboard" hint (`components/mic/is-ios.ts`). Test it on Chris's iPhone.
- **Open right now:** publish the **LinkedIn post** (draft + reviewer prompt in `docs/LINKEDIN_LAUNCH.md`; needs the people to tag and the `og-nest.png` image). The `public/brand/og-nest.png` file is still missing (1200x630 constellation screenshot or a code-based `app/opengraph-image.tsx`). Privacy stays **off the site** (Krava is Level-1 inference-TEE only, storage plaintext, prod routing unverified). The carryover QC backlog + the `service_role` -> `sb_secret_` migration (deprecated **end of 2026**) are in `PROGRESS.md`.
