...
- **LinkedIn launch prep (2026-06-05, live):** the landing now **leads with the community Nest** constellation, with an **inline waitlist + social-proof count** under it ("150+ curiosities across 15 subjects"), a "build your own community nest" hero CTA, **"Coming soon"** badges on Glints + Draw Out, a softened "Yours to keep" trust card, **og/twitter metadata** (the `/brand/og-nest.png` asset is still missing), a **"Join the waitlist" button in the in-app header**, and a footer **contact link to Chris's LinkedIn**. Community is the hook; the personal/memory/privacy moat stays. The **waitlist works on prod** (reads are dashboard/service-role only).
...
- **Open right now:** publish the **LinkedIn post** (draft + reviewer prompt prepared; needs the people to tag and the `og-nest.png` image). Privacy stays **off the site** (Krava is Level-1 inference-TEE only, app-key based, storage plaintext, prod routing unverified). The carryover QC backlog + the `service_role` -> `sb_secret_` migration (deprecated **end of 2026**) are in `PROGRESS.md`.
