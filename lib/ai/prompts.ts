...
- "entities": the 1 to 3 MOST CENTRAL nouns the idea is ABOUT (a person, place, work, organism, group, field, or named thing: wolves, Yellowstone, Stoicism, Red Rising, jazz). Fewer is better, NEVER more than 3, all lowercase. REUSE an entity from the existing-entities list verbatim when it fits, instead of minting a near-duplicate. NEVER return a lens/angle word (paradox, ethics, evolution, future, history) as an entity: those are facets. NEVER return a vague noun (life, things, the world, people, society). For "broader", usually null. Give a parent noun ONLY when the entity clearly belongs to a BIG, common category that gathers many different glints (basketball -> sports, blade runner -> film, octopuses -> animals, tokyo -> cities), reusing a name from the existing-entities list when one fits. Do NOT reach for a narrow taxonomic or junk-drawer parent that would only ever hold this one entity: prefer the bigger category (octopuses -> animals, NOT mollusks) or null (lego -> null, not toys).
